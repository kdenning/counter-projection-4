...
The majority of tests suggest only 1 factor, followed by 2 or 4 latent factors. 2 latent factors may make more theoretical sense for our data, but we will look at both.
...
efa_3fac <- psych::fa(EFA, nfactors = 3) %>% 
  model_parameters(sort = TRUE, threshold = "max")
efa_3fac
```

The EFA with 2 factors explains more of the total variance of the original data. Just for fun, I also tried 3 factors, and it worsened the explanation and none of the variables were used in the third factor. The 4 factor model did not converge.
