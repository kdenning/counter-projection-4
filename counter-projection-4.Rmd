...
# drops 2 participants due to missing data in identification here
EDBFI_data %>% 
  select(sub_id) %>% 
  unique() %>% 
  nrow()
...
People use more stereotypes of their in-group more than their out-group. This does not support our Confirmatory Prediction #2: "People will stereotype the out-group target more than the in-group target due to out-group homogeneity."  
...
The interaction of target group and identification on projection is significant. The main effect of target group is still significant. 
...
**To lessen the number of models run and the potential for spurious significant effects, we put all moderators in the same model, but only interacting with self and group (i.e., the moderators did not interact with each other). The model did converge, so we will use it.**
...
Even with moderators, we still have a significant main effect of group. All other significant main effects and interactions on projection are listed below.

Sig. main effects on projection:

* Threat
* Fundamental moral rights

Sig. interactions:
* Group x perceived polarization
* Group x fundamental moral rights

## ## Exploratory Moderator Results with stereo

```{r model 4 stereo}
indivdiff_data_stereo <- indivdiff_data %>% 
  select(-stereo, -perceive_polar, - contentiousness, -stereo, -self,
         -lib, -cons, -fundaMoral, -threat, -identification) %>% 
  na.omit()

model4_stereo <- lmer(Targ ~ group_cond*self_c*perceive_polar_c*stereo_c +
                          group_cond*self_c*threat_c*stereo_c +
                          group_cond*self_c*content_c*stereo_c +
                          group_cond*self_c*fundaMoral_c*stereo_c +
                        + (self_c|sub_id),
               data = indivdiff_data_stereo)

summary(model4_stereo)
```

Even with moderators and stereotyping, we still have a significant main effect of group, BUT we will need to plot it to see direction (though the estimate is negative). All other significant main effects and interactions are listed below. The significant effects on stereotyping solely (no projection involved) are listed, but not plotted due to interest and sake of researcher time.

Sig. main effects on projection:
* Threat


Sig. interactions on projection:
* Group x stereotyping
* Group x perceived polarization x stereotyping


Marginal effects projection:
* Stereotyping main effect on projection at .052
* Fundamental moral rights at .09

Sig main effects on stereo:
* Perceived polarization
* Threat
* Fundamental moral rights

Sig. interactions on stereo:

Marginal effects stereo:
* Group
* Threat x group
...
model4_stereo_fm <- lmer(Targ ~ group_cond*self_c*fundaMoral_c*stereo_c +
                        + (self_c|sub_id),
               data = indivdiff_data_stereo)

summary(model4_stereo_fm)
