...
# Pre-reg Mod 1: Projection by group
...
## Using Everyday Likes (ED)

```{r model 1 ED data}
model1ED_data <- clean_ptcheck %>% 
...
```{r}
contrasts(model1ED_data$group_cond) <- contr.sum(2)
contrasts(model1ED_data$group_cond)
````

```{r model 1 ED results}
model1ED <- lmer(TargED ~ selfED_c*group_cond + (selfED_c|sub_id),
               data = model1ED_data)
summary(model1ED)
```
...
```{r model 1 ED prep main effect group}
mod1ED_proj_group <- effect(term = "selfED_c:group_cond", 
                           xlevels = list(group_cond = c("In-group", "Out-group")), 
                           mod = model1ED)

mod1ED_proj_group <- as.data.frame(mod1ED_proj_group)
```

```{r Model 1 ED main effect group plot}
ggplot(mod1ED_proj_group, aes(selfED_c, fit, group = group_cond)) +
  geom_smooth(method = "lm", 
              se = FALSE, 
              size = .7, 
              colour = "black", 
  ...
  theme(legend.key.size = unit(1, "cm"),
        legend.title = element_blank()) +
  labs(title = "Plot from Model 1: Main effect of target group on projection",
       subtitle = "Projection using 'Everyday Likes' items",
         x = "Everyday likes for self",
         y = "Everyday likes for target")
```

## Using BFI + ED combined
...
```{r model 1 EDBFI contrasts}
contrasts(EDBFI_data$group_cond) <- contr.sum(2)
contrasts(EDBFI_data$group_cond)
````

```{r model 1 EDBFI results}
model1EDBFI <- lmer(Targ ~ self_c*group_cond + (self_c|sub_id),
               data = EDBFI_data)
summary(model1EDBFI)
```
...
```{r model 1 EDBFI prep main effect group}
mod1EDBFI_proj_group <- effect(term = "self_c:group_cond", 
                           xlevels = list(group_cond = c("In-group", "Out-group")), 
                           mod = model1EDBFI)

mod1EDBFI_proj_group <- as.data.frame(mod1EDBFI_proj_group)
```

```{r Model 1 EDBFI main effect group plot}
ggplot(mod1EDBFI_proj_group, aes(self_c, fit, group = group_cond)) +
  geom_smooth(method = "lm", 
              se = FALSE, 
              size = .7, 
              colour = "black", 
...
# Pre-reg Mod 3: Stereo as moderator

## Using BFI

**Added stereotyping to Model 1 to see if it adding it changed counter projection.**
...
describe(model3_data$stereoBFI_c)

mod3_proj_group <- effect(term = "selfBFI_c:group_cond", 
                           xlevels = list(group_cond = c("In-group", "Out-group")), 
                           mod = model3)

mod3_proj_group <- as.data.frame(mod3_proj_group)
```

```{r Model 3 main effect group plot}
ggplot(mod3_proj_group, aes(selfBFI_c, fit, group = group_cond)) +
  geom_smooth(method = "lm", 
              se = FALSE, 
              size = .7, 
              colour = "black", 
  ...
  theme(legend.key.size = unit(1, "cm"),
        legend.title = element_blank()) +
  labs(title = "Plot from Model 3: Main effect of target group on projection",
...
### Plot: stereo * group int

```{r model 3 interaction with stereo prep}
describe(model3_data$stereoBFI_c)

mod3_proj_stereo_effects <- effect(term = "selfBFI_c:group_cond:stereoBFI_c", 
                           xlevels = list(group_cond = c("In-group", 
                                                         "Out-group"), 
                                         stereoBFI_c = c(-1.22, 0, 1.22)), 
                           mod = model3)

mod3_proj_stereo_effects <- as.data.frame(mod3_proj_stereo_effects)

mod3_proj_stereo_effects %<>% 
...
```{r model 3 interaction plot with stereo}
ggplot(mod3_proj_stereo_effects, aes(selfBFI_c, 
                                     fit, 
                                     group = group_cond,
                                     colour = group_cond)) +
  ...
  theme_minimal() +
  theme(legend.title = element_blank()) +
  labs(title = "Plot from Model 3: Projection predicted by stereotype use and group membership",
...
```{r model 3 ED results, warning = FALSE, message = FALSE, error = FALSE}
model3ED <- lmer(TargED ~ stereoED_c*group_cond*selfED_c + (selfED_c|sub_id),
               data = model3ED_data)

summary(model3ED)
```

Model failed to converge with this few nested data-points within sub-id for both self and stereo. Results are not reliable.

## Using ED + BFI

```{r model 3 EDBFI results}
model3EDBFI <- lmer(Targ ~ stereo_c*group_cond*self_c + (self_c|sub_id),
               data = EDBFI_data)

summary(model3EDBFI)
```

When combined, the significant three-way interaction disappears, but the effect of group on projection is still significant. There is also now a significant effect of group on stereotyping that did not exist with just the BFI. Both are plotted below.
...
People stereotype their in-group more than their out-group. Interestingly, when projection is included in the model, the y-intercept for more negative scores is not as close as it was in Model 2, where projection was not included in the model. 
...
mod3EDBFI_projplotdata <- effect(term = "group_cond:self_c", 
                           xlevels = list(group_cond = c("In-group", 
                                                         "Out-group")), 
                           mod = model3EDBFI)

mod3EDBFI_projplotdata <- as.data.frame(mod3EDBFI_projplotdata)
```

```{r model 3 plot projection EDBFI}
ggplot(mod3EDBFI_projplotdata, aes(self_c, fit, group = group_cond)) +
  geom_smooth(method = "lm", 
              se = FALSE, 
              size = .7,
              colour = "black", 
...
When everyday likes items are added to the BFI, even with stereotyping controlled for in the model, we see counter projection with the out-group target.

# Pre-reg Mod 4: Identification as moderator

```{r model 4}
model4_data <- clean_ptcheck %>% 
  select(sub_id, BFI_number, BFI_target_group, TargBFI, selfBFI_c, group_cond, 
...
contrasts(model4_data$group_cond) <- contr.sum(2)
contrasts(model4_data$group_cond)
```

```{r model 4 results without stereo}
model4_nostereo <- lmer(TargBFI ~ selfBFI_c*group_cond*identification_c + 
                          (selfBFI_c|sub_id),
               data = model4_data)
...
```{r model 4 no stereo group by ident plot prep}
describe(model4_data$identification_c)

mod4_proj_ident_effects <- effect(term = "selfBFI_c:group_cond:identification_c", 
                           xlevels = list(group_cond = c("In-group", 
                                                         "Out-group"), 
                                         identification_c = c(-1.27, 0, 1.27)), 
                           mod = model4_nostereo)

mod4_proj_ident_effects <- as.data.frame(mod4_proj_ident_effects)

mod4_proj_ident_effects %<>% 
...
```{r model 4 no stereo group by ident plot}
ggplot(mod4_proj_ident_effects, aes(selfBFI_c, 
                                     fit, 
                                     group = group_cond,
                                     colour = group_cond)) +
  geom_smooth(method = "lm", se = FALSE, size = .7, colour = "black",
  ...
  theme_minimal() +
  theme(legend.title = element_blank()) +
  labs(title = "Plot Model 4 no stereotyping: Projection predicted by identification and group membership",
       x = "Personality traits for self",
       y = "Personality traits for target")
```
...
## With stereo in model

```{r model 4 results}
model4 <- lmer(TargBFI ~ selfBFI_c*group_cond*stereoBFI_c*identification_c + (selfBFI_c|sub_id),
               data = model4_data)
summary(model4)
```
...
```{r model 4 prep main effect group}
mod4_proj_group <- effect(term = "selfBFI_c:group_cond", 
                           xlevels = list(group_cond = c("In-group", "Out-group")), 
                           mod = model4)

mod4_proj_group <- as.data.frame(mod4_proj_group)
```

```{r Model 4 main effect group plot}
ggplot(mod4_proj_group, aes(selfBFI_c, fit, group = group_cond)) +
  geom_smooth(method = "lm", 
              se = FALSE, 
              size = .7, 
              colour = "black", 
  ...
  theme(legend.key.size = unit(1, "cm"),
        legend.title = element_blank()) +
  labs(title = "Plot Model 4 with stereotyping: Main effect of target group on projection",
       subtitle = "Including moderators stereotyping and in-group identification",
         x = "Personality responses for self",
         y = "Personality responses for target")
```
