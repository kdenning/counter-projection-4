...
model1_data <- clean_ptcheck %>% 
  ...
  na.omit() %>% 
  unique() ##### MK: OK, this makes so much sense now! After all those pivot_longers in data cleaning, I was a little confused.
```
...
##### MK: Just to compare the plots because there was a part I couldn't understand in the function. Looks like the function works nicely.
...
# Hierarchical build-up to Pre-reg Mod 2
...
ggplot(mod3EDBFI_nostereo_proj_ident, aes(self_c, fit, group = Identification)) +
  ...
  labs(title = "Model 3: Main effect of identification on projection controlling for group",
       subtitle = "Using 'Everyday Likes' + BFI items when stereotyping removed",
         x = "ED + BFI items for self",
         y = "ED + BFI items for target") ##### MK: It's kind of hard to distinguish the lines on the plot.
```
...
### Plot: Marginal Int Stereo & Ident on Projection
...
#### Stereo by group & identification
