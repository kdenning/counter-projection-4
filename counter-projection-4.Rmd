...
model2ED <- lmer(TargED ~ selfED_c*group_cond*stereoED_c + (selfED_c|sub_id),
               data = model2ED_data)
summary(model2ED)
```
...
model2EDBFI <- lmer(Targ ~ self_c*group_cond + (self_c|sub_id),
               data = EDBFI_data)
summary(model2EDBFI)
```
...
```{r model 2 EDBFI prep main effect group}
mod2EDBFI_proj_group <- effect(term = "self_c:group_cond", 
                           xlevels = list(group_cond = c("In-group", "Out-group")), 
                           mod = model2EDBFI)

mod2EDBFI_proj_group <- as.data.frame(mod2EDBFI_proj_group)
```

```{r Model 2 EDBFI main effect group plot}
ggplot(mod2EDBFI_proj_group, aes(self_c, fit, group = group_cond)) +
  geom_smooth(method = "lm", 
              se = FALSE, 
              size = .7, 
              colour = "black", 
...
model2EDBFI <- lmer(Targ ~ stereo_c*group_cond*self_c + (self_c|sub_id),
               data = EDBFI_data)
...
People stereotype their in-group more than their out-group. This is the opposite of our Confirmatory Prediction #2: "People will stereotype the out-group target more than the in-group target due to out-group homogeneity."  
...
mod2EDBFI_projplotdata <- effect(term = "group_cond:self_c", 
                           xlevels = list(group_cond = c("In-group", 
                                                         "Out-group")), 
                           mod = model2EDBFI)

mod2EDBFI_projplotdata <- as.data.frame(mod2EDBFI_projplotdata)
```

```{r model 2 plot projection EDBFI}
ggplot(mod2EDBFI_projplotdata, aes(self_c, fit, group = group_cond)) +
  geom_smooth(method = "lm", 
              se = FALSE, 
              size = .7,
              colour = "black", 
...
ggplot(mod3EDBFI_nostereo_proj_group, aes(self_c, fit, group = group_cond)) +
  ...
  labs(title = "Model 3: Main effect of target group on projection",
       subtitle = "Using 'Everyday Likes' + BFI items controlling for stereotyping",
         x = "ED + BFI items for self",
         y = "ED + BFI items for target")
```
...
model4EDBFI <- lmer(Targ ~ stereo_c*group_cond*self_c*identification_c + (self_c|sub_id),
               data = EDBFI_data)
...
```{r model 4 EDBFI stereo plot prep}
mod4EDBFI_plotdata <- effect(term = "stereo_c:group_cond", 
                           xlevels = list(group_cond = c("In-group", 
                                                         "Out-group"),
                                          stereo_c = c(-1.18, 0, 1.18)), 
                           mod = model4EDBFI)

mod4EDBFI_plotdata <- as.data.frame(mod4EDBFI_plotdata)
```

```{r model 4 plot stereo EDBFI}
ggplot(mod4EDBFI_plotdata, aes(stereo_c, fit, group = group_cond)) +
  geom_smooth(method = "lm", 
              se = FALSE, 
              size = .7, 
              colour = "black", 
              aes(linetype = group_cond)) +
  theme_minimal() +
  labs(title = "Plot from Model 4: Effect of target group on stereotyping",
       subtitle = "Higher correspondence between BFI + ED ratings indicates more stereotyping",
       y = "BFI + ED ratings for target",
       x = "BFI + ED ratings for stereotypical group member") +
   theme(legend.title = element_blank())
...
```{r model 4 EDBFI projection plot prep}
describe(EDBFI_data$self_c)

mod4EDBFI_projplotdata <- effect(term = "group_cond:self_c", 
                           xlevels = list(group_cond = c("In-group", 
                                                         "Out-group")), 
                           mod = model4EDBFI)

mod4EDBFI_projplotdata <- as.data.frame(mod4EDBFI_projplotdata)
```

```{r model 4 plot projection EDBFI}
ggplot(mod4EDBFI_projplotdata, aes(self_c, fit, group = group_cond)) +
  geom_smooth(method = "lm", 
              se = FALSE, 
              size = .7,
              colour = "black", 
              aes(linetype = group_cond)) +
  theme_minimal() +
  labs(title = "Plot from Model 4: Effect of target group on projection",
       subtitle = "Controlled for stereotyping",
       y = "BFI + ED ratings for target",
       x = "BFI + ED ratings for stereotypical group member") +
   theme(legend.title = element_blank())
...
Both projection and stereotyping plots are the same as the results from Model 3 using ED and BFI, indicating that adding identification into the model with the combined scales had not impact on the main effects of projection and stereotyping by group.
