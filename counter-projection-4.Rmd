...
model2_data <- clean_ptcheck %>% 
  select(sub_id, BFI_number, BFI_target_group, stereoBFI, selfBFI_c, group_cond, 
         opin_pol_combined) %>% 
  na.omit() %>% 
  unique()
```
...
```{r model 2 results}
model2 <- lm(stereoBFI ~ group_cond,
               data = model2_data)
summary(model2)
```
...
```{r}
mod2_plotdata <- effect(term = "group_cond", 
...
```{r model 2 plot}
ggplot(mod2_plotdata, aes(group_cond, fit)) +
  geom_col() +
  theme_minimal() +
  labs(title = "Effect of target group on stereotyping",
       subtitle = "Stereotyping measured by ratings on BFI for stereotypical group member",
       x = "Target group",
       y = "Stereotyping")
...
mod3_proj_stereo_effects$Stereotyping <- as.factor(mod3_proj_stereo_effects$stereoBFI_c)


stereo_label <- c("-1.22" = "Low stereotyping \n (1 SD below)", 
...
ggplot(mod3_proj_stereo_effects, aes(selfBFI_c, 
  ...
  labs(title = "Projection predicted by stereotype use and group membership",
