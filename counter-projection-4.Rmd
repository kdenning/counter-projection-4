...
model1_plot + 
  labs(title = "Model 1: Projection predicted by target group",
...
model2group_plot_EDnostereo +
  labs(title = "Exploratory Model 2: Projection predicted by target group",
       subtitle = "Projection using ELI",
       x = "ELI items for self",
       y = "ELI items for target",
       linetype = "Group")
...
model2group_plot_EDwstereo +
  labs(title = "Exploratory Model 2: Projection predicted by target group",
       subtitle = "Projection using ELI controlled for stereotyping",
       x = "ELI items for self",
       y = "ELI items for target",
       linetype = "Group")
...
model2group_plot_bfistereo +
  labs(title = "Exploratory Model 2: Projection predicted by target group",
       subtitle = "Projection using BFI controlled for stereotyping",
       x = "Personality responses for self",
       y = "Personality responses for target",
       linetype = "Group")
...
ggplot(model2_bfistereo_int_effects, aes(self_c, 
  ...
  theme_minimal() +
  labs(title = "Exploratory Model 2: Projection predicted by stereotype use and target group",
       subtitle = "Projection using BFI only",
...
ggplot(mod2EDBFI_targ_stereo_effects, aes(stereo_c, fit, group = group_cond)) +
  geom_smooth(method = "lm", 
              ...
              aes(linetype = group_cond)) +
  theme_minimal() +
  labs(title = "Model 2: NS Effect of target group on stereotyping",
...
model2group_plot_EDBFI_stereo +
  labs(title = "Model 2: Projection predicted by target group",
       subtitle = "Projection using BFI + ELI and controlled for stereotyping",
       y = "BFI + ELI ratings for target",
       x = "BFI + ELI ratings for self",
       linetype = "Group") 
...
```{r model 3 bfi contrasts, include = FALSE}
contrasts(dataBFI_mod3$group_cond) <- contr.sum(2)
contrasts(dataBFI_mod3$group_cond)
```
...
ggplot(mod3BFInostereo_proj_ident_group_effects, aes(self_c, 
  ...
  theme_minimal() +
  labs(title = "Model 3: Projection predicted by identification and target group",
       subtitle = "Pre-registered model used BFI and stereotyping was removed for power",
       x = "BFI for self",
       y = "BFI for target",
       linetype = "Group")
...
mod3BFI_nostereo_proj_group_plot +
  labs(title = "Model 3: Projection predicted by group",
       subtitle = "Pre-registered model used BFI and stereotyping was removed for power",
...
ggplot(mod3BFI_nostereo_proj_ident, aes(self_c, fit, group = Identification)) +
  ...
  theme_minimal(base_size = 13) +
  labs(title = "Model 3: Projection predicted by identification",
       subtitle = "Pre-registered model used BFI and stereotyping was removed for power",
         x = "BFI items for self",
         y = "BFI items for target") +
...
```{r model 3 ed contrasts, include = FALSE}
contrasts(dataED_mod3$group_cond) <- contr.sum(2)
contrasts(dataED_mod3$group_cond)
```
...
ggplot(mod3EDnostereo_proj_ident_group_effects, aes(self_c, 
  ...
  facet_grid(cols = vars(Identification)) +
  theme_minimal() +
  labs(title = "Exploratory Model 3: Projection by identification and target group",
       subtitle = "Projection using ELI",
       x = "ELI for self",
       y = "ELI for target",
       linetype = "Group")
...
ggplot(mod3EDstereo_4way_effects, aes(self_c, 
  ...
  theme_minimal() +
  theme(legend.key.size = unit(1, "cm")) +
  labs(title = "Exploratory Model 3: Projection by stereotyping, identification, and target group",
       subtitle = "Projection using ELI items",
       x = "ELI items for self",
       y = "ELI items for target",
       linetype = "Group")
...
ggplot(mod4_polar_ME_effects, aes(self_c, 
  ...
  theme_minimal() +
  theme(legend.key.size = unit(1, "cm")) +
  labs(title = "Projection predicted by polarization",
       subtitle = "Exploratory model with projection using ELI items",
       x = "ELI items for self",
       y = "ELI items for target",
...
ggplot(mod4_polar_effects, aes(self_c, 
  ...
  theme_minimal() +
  theme(legend.key.size = unit(1, "cm")) +
  labs(title = "Projection predicted by target group and polarization",
       subtitle = "Exploratory model with projection using ELI items",
       x = "ELI items for self",
       y = "ELI items for target",
       linetype = "Group")
...
# BFI broken down into facets

## Extraversion

```{r extra data}
extra_mod_data <- clean_ptcheck %>% 
  select(sub_id, BFI_number, TargBFI, selfBFI_c, stereoBFI_c, group_cond, 
         identification_c) %>% 
  filter(BFI_number == 1 | BFI_number == 6 | BFI_number == 11) %>% 
  rename(self_c = selfBFI_c) %>% 
  unique() %>% 
  na.omit()

contrasts(extra_mod_data$group_cond) <- contr.sum(2)
```

### Results

```{r extra model}
extra_model <- lmer(TargBFI ~ self_c*group_cond*stereoBFI_c  +
                        + (self_c|sub_id),
               data = extra_mod_data)
...
```{r extra plot}
extra_plot <- get_groupME_proj_plot(extra_model)

extra_plot + 
  labs(title = "Projection predicted by target group",
       subtitle = "Projection using extraversion controlled for stereotyping",
       x = "Extraversion for self",
       y = "Extraversion for target",
       linetype = "Group")
```

Effect was marginal only

## Agreeableness

```{r agree data}
agree_mod_data <- clean_ptcheck %>% 
  select(sub_id, BFI_number, TargBFI, selfBFI_c, stereoBFI_c, group_cond, 
         identification_c) %>% 
...
contrasts(agree_mod_data$group_cond) <- contr.sum(2)
```

### Results

```{r agree model}
agree_model <- lmer(TargBFI ~ self_c*group_cond*stereoBFI_c  +
                        + (self_c|sub_id),
               data = agree_mod_data)
...
```{r agree plot set-up}
describe(agree_mod_data$stereoBFI_c)

agree_effects <- effect(term = "self_c:group_cond:stereoBFI_c", 
                           xlevels = list(group_cond = c("In-group", 
                                                         "Out-group"),
                                          stereoBFI_c = c(-1.33, 0, 1.33)), 
                           mod = agree_model)

agree_effects <- as.data.frame(agree_effects)

agree_effects %<>% 
  mutate(Stereotyping = as.factor(stereoBFI_c),
         Stereotyping = dplyr::recode(Stereotyping,
                                      `-1.33` = "Low stereotyping \n (1 SD below)",
                                      `0` = "Average \n stereotyping",
                                      `1.33` = "High stereotyping \n (1 SD above")) 

```

```{r agree plot}
ggplot(agree_effects, aes(self_c, 
                                      fit, 
                                      group = group_cond, 
                                      colour = group_cond)) +
  geom_smooth(method = "lm", se = FALSE, size = .7, colour = "black",
  ...
  theme_minimal() +
  theme(legend.key.size = unit(1, "cm")) +
  labs(title = "Projection predicted by stereotyping and target group",
       subtitle = "Projection using agreeableness",
       x = "Agreeableness for self",
       y = "Agreeableness for target",
       linetype = "Group")
```
...
contrasts(negemo_data$group_cond) <- contr.sum(2)
```

### Results

```{r negemo model}
negemo_model <- lmer(TargBFI ~ self_c*group_cond*stereoBFI_c  +
                        + (self_c|sub_id),
               data = negemo_data)

summary(negemo_model)
```

No results with projection

## Open minded

```{r open minded data}
open_data <- clean_ptcheck %>% 
  select(sub_id, BFI_number, TargBFI, selfBFI_c, stereoBFI_c, group_cond, 
         identification_c) %>% 
...
contrasts(open_data$group_cond) <- contr.sum(2)
```

### Results

```{r open model}
open_model <- lmer(TargBFI ~ self_c*group_cond*stereoBFI_c  +
                        + (self_c|sub_id),
               data = open_data)
...
```{r open plot set-up}
describe(open_data$stereoBFI_c)

open_effects <- effect(term = "self_c:group_cond:stereoBFI_c", 
                           xlevels = list(group_cond = c("In-group", 
                                                         "Out-group"),
                                          stereoBFI_c = c(-1.28, 0, 1.28)), 
                           mod = open_model)

open_effects <- as.data.frame(open_effects)

open_effects %<>% 
  mutate(Stereotyping = as.factor(stereoBFI_c),
         Stereotyping = dplyr::recode(Stereotyping,
                                      `-1.28` = "Low stereotyping \n (1 SD below)",
                                      `0` = "Average \n stereotyping",
                                      `1.28` = "High stereotyping \n (1 SD above")) 

```

```{r open plot}
ggplot(open_effects, aes(self_c, 
                                      fit, 
                                      group = group_cond, 
                                      colour = group_cond)) +
  geom_smooth(method = "lm", se = FALSE, size = .7, colour = "black",
  ...
  theme(legend.key.size = unit(1, "cm")) +
  labs(title = "Projection predicted by stereotyping and target group",
       subtitle = "Projection using Openness",
       x = "Openness for self",
       y = "Openess for target",
       linetype = "Group")
```
...
contrasts(consc_data$group_cond) <- contr.sum(2)
```

### Results

```{r consc model}
consc_model <- lmer(TargBFI ~ self_c*group_cond*stereoBFI_c  +
                        + (self_c|sub_id),
               data = consc_data)
...
```{r consc plot}
consc_plot <- get_groupME_proj_plot(consc_model)

consc_plot + 
  labs(title = "Projection predicted by target group",
       subtitle = "Projection using conscientiousness controlled for stereotyping",
...
contrasts(honesty_data$group_cond) <- contr.sum(2)
```

### Results

```{r honesty model}
honesty_model <- lmer(TargBFI ~ self_c*group_cond*stereoBFI_c  +
                        + (self_c|sub_id),
               data = honesty_data)
...
```{r honesty plot}
honesty_plot <- get_groupME_proj_plot(honesty_model)

honesty_plot + 
  labs(title = "Projection predicted by target group",
       subtitle = "Projection using honesty/morality traits controlled for stereotyping",
       x = "Honesty/Morality for self",
       y = "Honesty/Morality for target",
       linetype = "Group")
```
