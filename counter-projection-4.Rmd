...
### Correlation matrix for ED scale
...
# Pre-reg Mod 3: Identification as moderator {.tabset .tabset-fade .tabset-pills}

## Confirmatory Model: W/o stereotyping for power
...
model3EDBFI_nostereo <- lmer(Targ ~ self_c*group_cond*identification_c + (self_c|sub_id),
               data = EDBFI_mod3)

summary(model3EDBFI_nostereo)
confint(model3EDBFI_nostereo)
```
...
describe(EDBFI_mod3$identification_c)

mod3nostereo_proj_ident_effects <- effect(term = "self_c:group_cond:identification_c", 
                           xlevels = list(group_cond = c("In-group", 
                                                         "Out-group"), 
...
mod3EDBFI_nostereo_proj_group_plot <- get_groupME_proj_plot(model3EDBFI_nostereo)

mod3EDBFI_nostereo_proj_group_plot +
...
On lower scores for ED + BFI, there is less difference in projection between identification groups. As ED and BFI scores increase, you see more projection in high identifiers compared to average and then low identifiers.

## Exploratory Model: W/ stereotyping

### Results

```{r model 3 EDBFI results}
model3EDBFI_stereo <- lmer(Targ ~ self_c*group_cond*stereo_c*identification_c + (self_c|sub_id), data = EDBFI_mod3)

summary(model3EDBFI_stereo)
confint(model3EDBFI_stereo)
```

When BFI and ED are combined and added to a model controlled by stereotyping, the interaction between condition and identification on projection disappears. There is a significant main effect of identification on stereotyping as well as group on projection. There is also a significant interaction of group and identification on stereotyping.There is a marginal interaction of stereotyping and identification on projection collapsed across group

### Plots

```{r model 3 EDBFI projection plot}
mod3EDBFI_stereo_group_plot <- get_groupME_proj_plot(model3EDBFI_stereo)

mod3EDBFI_stereo_group_plot +
...
describe(EDBFI_mod3$stereo_c)

mod3stereo_3way_effects <- effect(term = "self_c:stereo_c:identification_c", 
                           xlevels = list(identification_c = c(-1.27, 0, 1.27),
...
Collapsed across in and out-group target, people who apply less stereotypes to the target project more as in-group identification increases. Interestingly, people who demonstrate high use of stereotypes to the target exhibit the opposite pattern: low in-group identifiers project the most and as in-group identification increases, projection decreases. Average stereotype users all project around the same amount.
...
mod3stereobygroupident <- effect(term = "group_cond:stereo_c:identification_c", 
                           xlevels = list(identification_c = c(-1.27, 0, 1.27),
...
This effect was not significant. We plotted for comparison.
...
This interaction is driven by the fact that, when people perceive low polarization, they project with their out-group. However, as polarization increases, counter projection with the out-group target increases. The intercept of positive projection also increases with the in-group as polarization increases.
...
mod4_fundmoral_effects %<>% 
  mutate(fundaMoral = as.factor(fundaMoral_c),
         fundaMoral = dplyr::recode(fundaMoral,
                                `-1.17` = "Low importance to \n fundamental beliefs",
                                `0` = "Average \n importance",
                                `1.17` = "High importance to \n fundamental beliefs")) 
```
...
ggplot(mod4_fundmoral_effects, aes(self_c, 
  ...
  theme(legend.key.size = unit(1, "cm"),
        legend.title = element_blank()) +
  labs(title = "Model 4: Projection predicted by fundamental beliefs and group",
       subtitle = "Projection using ED + BFI items",
       x = "ED + BFI items for self",
       y = "ED + BFI items for target")
```
