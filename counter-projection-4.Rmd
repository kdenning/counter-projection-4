...
clean_data <- wide_data %>% 
  ...
  mutate(perceive_polar = abs(lib - cons)) %>% #getting variable for perceived polar by subtracting perceived polarization for conservatives from liberals and getting teh absolute value of that diff
...
ggplot(mod3stereo_3way_effects, aes(self_c, 
  ...
  labs(title = "Projection by stereotyping and identification",
  theme_sjplot() +
  theme(legend.key.size = unit(1, "cm"),
        legend.title = element_blank()) +
...
The EFA with 2 factors explains more of the total variance of the original data (48%). The 4 factor then explained the next highest amount of total variance (42%). Our singly factor latent variable EFA explained only 33% of the total variance.

It appears that the 2 factor structure is our best structure. This structure also combines threat & contentiousness into one factor and fundamental morals, identification, and perceived polarization into another factor. These factors seem to fall into two separate categories: (1) Aggression, (2) Group beliefs.
...
model4_nostereo <- lmer(Targ ~ group_cond*self_c*perceive_polar +
                          group_cond*self_c*
                        + (self_c|sub_id),
               data = EDBFI_overall)
