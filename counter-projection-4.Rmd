...
```{r EFA check}
EFA <- EDBFI_overall %>% 
  select(identification, threat, perceive_polar, fundaMoral, contentiousness) %>% 
  unique()
...
The majority of tests suggest only 1 factor, followed by 2 latent factors. 2 latent factors may make more theoretical sense for our data, but we will look at both.
...
The EFA with 2 factors explains more of the total variance of the original data. Just for fun, I also tried 3 factors, and it worsened the explanation and none of the variables were used in the third factor. It appears that the 2 factor structure is our best structure. This structure also combines threat & contentiousness into one factor and fundamental morals, identification, and perceived polarization into another factor. These factors seem to fall into two separate categories: (1) Aggression, (2) Group beliefs.
